...
Xylar Asay-Davis, Phillip Wolfram
...
def plot_polar_comparison(
    ...
    dpi = 300): # {{{
    """
    Plots model versus observations for polar regions.
...
def plot_global_comparison(
    ...
    dpi = 300): # {{{
    """
    Plots model versus observations for global projections.
    ...
    # this doesn't work because lons are -180 to 180..
    #m = Basemap(projection='robin', lon_0=200, resolution='l')
    x, y = m(Lons, Lats) # compute map proj coordinates
...
def plot_moc(config, mocLat,
             ...
             dpi=300): #{{{
    """
    Plots comparison of moc section

    Mark Petersen, Phillip Wolfram
    Last modified: 01/23/2017
    """

    contourLines = config.getExpression('moc_postprocess',
                                        'contourLines', usenumpy=True)
